...
class CompressTab(ScrollFrame):
    ...
    def _build_profile_row(self) -> None:
        card = Card(self, title=t("Профиль"), subtitle=t("Готовый набор параметров качества"))
        ...
        button(body, t("Сохранить как…"), self._save_profile, width=150).grid(row=0, column=1)
        self.delete_profile_button = button(
            body, t("Удалить"), self._delete_profile, variant="danger", width=100
        )
    ...
    def _build_output_row(self) -> None:
        ...
        ctk.CTkLabel(body, text=t("Сохранить в"), anchor="w", width=110).grid(row=1, column=0, sticky="w")
    ...
    def _refresh_profiles(self) -> None:
        # Встроенные профили названы по-русски; в списке показываем перевод,
        # а ищем профиль по исходному имени (см. _profile_source).
        names = [t("Ручные настройки")] + [
            t(p.name) for p in self.app.profiles.list()
        ]
        self.profile_menu.configure(values=names)
        self._update_delete_button()

    def _on_profile_selected(self, name: str) -> None:
        if name == t("Ручные настройки"):
    ...
    def _delete_profile(self) -> None:
        ...
        self._refresh_profiles()
        self.profile_menu.set(t("Ручные настройки"))
        self._container_override = None
        self._update_delete_button()

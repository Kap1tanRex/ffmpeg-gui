...
#: Доступные масштабы интерфейса: подпись -> множитель.
UI_SCALES: dict[str, float] = {
    "100 %": 1.0,
    "110 %": 1.1,
    "125 %": 1.25,
    "150 %": 1.5,
...
#: За эти границы масштаб не выпускается: ниже интерфейс нечитаем, выше он
#: перестаёт помещаться в окно даже на большом экране.
SCALE_MIN, SCALE_MAX = 0.8, 2.0


def scale_label(value: float) -> str:
    """Множитель -> подпись из :data:`UI_SCALES`, ближайшая по значению."""
    return min(UI_SCALES, key=lambda label: abs(UI_SCALES[label] - value))


def set_ui_scale(value: float) -> None:
    """Масштаб всего интерфейса: размеры виджетов и шрифтов сразу.

    CustomTkinter пересчитывает уже созданные виджеты, поэтому перезапуск не
    нужен. Отступы в сетке (``padx``/``pady``) — обычные пиксели Tk, их
    масштабирование не касается, поэтому при больших значениях интерфейс
    становится плотнее, а не просто крупнее.

    Размер самого окна намеренно не трогается. ``set_window_scaling`` растянул
    бы его в те же полтора раза, и на экране 1080p окно при 150 % перестало
    бы помещаться целиком. Вместо этого крупнее становится содержимое, а
    окно остаётся тем, каким его сделал пользователь: разделы прокручиваются.
    """
    scale = max(SCALE_MIN, min(SCALE_MAX, float(value or 1.0)))

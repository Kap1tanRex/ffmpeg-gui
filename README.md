...
Списков кодеков и контейнеров в коде нет: программа спрашивает у установленного
FFmpeg, что он умеет, а аппаратные энкодеры вдобавок пробует запустить. В списке
оказывается ровно то, что работает на этой машине, — а не то, что оказалось
скомпилировано в сборку. Про неработающий NVENC вы узнаете сразу, а не после
того, как задание упало.
...
**Не бросает на полпути.** Аппаратные энкодеры проверяются по-настоящему: при
первом запуске программа пробует закодировать ими один кадр и предлагает только
те, что действительно завелись. В списке `-encoders` любой сборки FFmpeg для
Windows есть и NVENC, и Quick Sync, и AMF — но заведётся тот, под который есть
живой драйвер. Версия драйвера видеокарты определяется сама и видна в строке
состояния внизу окна — при разборе отказа NVENC это первое, что спрашивают.
Если видеокарта всё же отказала посреди работы, задание не падает с сообщением
про NVENC, а само повторяется на процессоре.

Результат сначала пишется во временный файл, проверяется, и только потом
занимает своё место: неудачное кодирование не испортит то, что уже лежало на
диске.
...
Язык — русский или английский. Переключается в настройках и применяется при
следующем запуске: подписи виджетов читаются в момент постройки окна.
...
| `resources/translations/` | `en.json` — словарь «русская строка → английская» |
...
103 проверки, меньше секунды, без FFmpeg и без графической подсистемы: сборка
команды, шкала качества, шаблоны имён, разбор ошибок FFmpeg и откат задания на
процессор, определение видеокарты и драйвера, полнота перевода. Проверки интерфейса — прокрутка, подсказки, масштаб — гоняются
сценариями на живом окне и в репозиторий не входят.

---

## Чего пока нет

- Автоматических проверок интерфейса в репозитории нет: они требуют живого окна
